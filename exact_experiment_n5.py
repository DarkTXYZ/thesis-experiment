...
DATASET_DIR = "Dataset/exact_dataset_n5"
PKL_PATH = os.path.join(DATASET_DIR, "exact_dataset.pkl")
CSV_PATH = os.path.join("Results", "exact_experiment_n5_results.csv")
...
def main():
    ...
    for num_nodes in [3, 4, 5]:
        ...
        edgelist_files = sorted(
            f for f in os.listdir(subdir) if f.endswith(".edgelist")
        )
        print(f"\nProcessing {len(edgelist_files)} graphs from n{num_nodes}...\n")

        for filename in edgelist_files:
            graph_name_base = os.path.splitext(filename)[0]
            graph_name_key = f"n{num_nodes}_{graph_name_base}"  # Match the pickle key
            filepath = os.path.join(subdir, filename)
            graph = nx.read_edgelist(filepath, nodetype=int)
            n = graph.number_of_nodes()

            if graph_name_key not in ground_truth:
                print(f"  Warning: {graph_name_key} not found in ground truth, skipping...")
                continue

            gt = ground_truth[graph_name_key]
        ...
        print(f"\n{'='*50}")
        print(f"Graph: {graph_name_key} (n={n}, m={graph.number_of_edges()}), ground truth obj: {gt_obj}")

        # dimod ExactSolver
        exact_results = solve_with_exact(graph, bqm, n)
        report("dimod.ExactSolver", graph_name_key, graph, gt_obj, gt_solutions, exact_results)

...
def generate_connected_random_graph(n: int, density: float, max_attempts: int = 10000) -> nx.Graph:
...
def save_graphs_to_pickle(num_vertices_list: list, num_graphs: int, density: float, seed: int, output_dir: str):
    ...
    for n in num_vertices_list:
        ...
        print(f"  Avg edges: {avg_edges:.1f}, Avg density: {avg_density:.4f}")
        
        result = {
            'name': f"quantum_n{n}",
            'num_vertices': n,
            'num_graphs': len(graphs_data),  # Use actual number of graphs generated
            'density': density,
...
def main():
    seed = 42
    num_vertices_list = [5, 10, 15, 20, 25]
    num_graphs = 100  
    density = 0.5 

...
RESULTS_CSV = os.path.join(Path(__file__).parent.parent, "Results/PIM_tuning_fixed_temperature_20260331_100606.csv")

def load_results():
    """Load PIM tuning results from CSV"""
...
def print_result(row):
    ...
    print(f"  Space type: {row['space_type']}")
...
def get_top_feasible(n=10):
    ...
    for idx, (_, row) in enumerate(top.iterrows(), 1):
        print(f"{idx}. Cost={row['minla_cost']:4d} | Gap={row['relative_gap']:6.2%} | "
...
def get_top_infeasible(n=10):
    ...
    print("-" * 100)
    for idx, (_, row) in enumerate(top.iterrows(), 1):
        print(f"{idx}. Energy={row['energy']:12.6f} | "
              f"Beta={row['beta']:<7g} | {row['space_type']:9s} | "
              f"Norm={str(row['bqm_is_normalized']):5s} | Time={row['time_s']:6.2f}s")
    return top
...
def get_top_10_by_normalized(n=10):
    ...
    for norm_status in [True, False]:
        ...
        print("-" * 120)
        print(f"{'Rank':<5} {'Energy':<14} {'Feasible':<10} {'Cost':<8} {'Gap':<8} "
              f"{'Beta':<12} {'Space Type':<12} {'Time(s)':<8}")
        print("-" * 120)
        ...
        for idx, (_, row) in enumerate(top_n.iterrows(), 1):
            ...
            gap_str = f"{row['relative_gap']:.2%}" if (row['feasible'] and pd.notna(row['relative_gap'])) else "-"
            beta_val = f"{row['beta']:g}"
            
            print(f"{idx:<5} {row['energy']:<14.6f} {feasible_str:<10} {cost_str:<8} "
                  f"{gap_str:<8} {beta_val:<12} {row['space_type']:<12} {row['time_s']:<8.3f}")
